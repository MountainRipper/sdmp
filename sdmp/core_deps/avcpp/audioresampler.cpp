...
AudioResampler::AudioResampler(uint64_t dstChannelsLayout, int dstRate, SampleFormat dstFormat,
    ...
    : AudioResampler()
{
    init(dstChannelsLayout, dstRate, dstFormat, srcChannelsLayout, srcRate, srcFormat, options, nullptr, ec);
}
...
bool AudioResampler::init(uint64_t dstChannelsLayout, int dstRate, SampleFormat dstFormat,
                          uint64_t srcChannelsLayout, int srcRate, SampleFormat srcFormat,
                          Dictionary &options,  int* mapping,OptionalErrorCode ec)
{
...
bool AudioResampler::init(uint64_t dstChannelsLayout, int dstRate, SampleFormat dstFormat,
                          uint64_t srcChannelsLayout, int srcRate, SampleFormat srcFormat,
                          AVDictionary **dict, int* mapping,OptionalErrorCode ec)
{
    clear_if(ec);
    ...
    if(mapping){
        sts = swr_set_channel_mapping(m_raw,mapping);
        if (sts < 0)
            goto ffmpeg_internal_fails;
    }
