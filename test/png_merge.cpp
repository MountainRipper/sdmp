...
int main(int argc, char *argv[])
{
    ...
    for (int i=0; i< rects.size(); i++)
    {
        ...
        menifest["images"][image.file_path.filename()] = {{"x",rect.x},{"y",rect.y},{"w",image.width},{"h",image.height}};
    }
    ...
    menifest["image"]  = image_file;
    menifest["imageSize"]  = target_size;
    std::ofstream o((filename+".json").c_str());
    o << std::setw(2) << menifest << std::endl;
    return 0;
}
